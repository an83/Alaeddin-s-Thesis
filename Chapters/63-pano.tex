...
This work assumes that the panoramic 360-degree image was captured and stitched beforehand (offline). The Google Glass and the Android tablet are used to view and interact with the panoramic 360-degree image.
...
There have been technological advancements in AR headsets since this work completed. For instance, if HoloLens\footnote{\url{https://en.wikipedia.org/wiki/Microsoft_HoloLens}} was used, there will be an additional level of interaction involving 3D depth data (how far or close the annotations are from the user). Instead, a light AR device (Google Glass) was used without a 3D depth sensor – which meant the user had to be stationary (no moving forward or backward). Having the ability to get closer or further away from the annotation and panoramic seen may increase social presence because of more involvement from the users.
