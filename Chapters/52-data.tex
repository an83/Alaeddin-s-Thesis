...
Alternative options for future studies in displaying 360-degree video on AR display include: 1) Combine 3D depth data with the 360 video, then using a depth camera to determine the current viewing scene in order to overlay the 360 image on top of the physical world properly. 2) Unwrap the 360 video/image into equitangular projection into a 2D surface. 3) Enable the wearable device to be able to switch into a video see-through display for when the user is viewing a 360 image and stationary (not moving).

Nonetheless if the sharer is sharing a photo is focused on what the Viewer wants to see, then it will be more efficient than a 360 image or a 3D scan. However, the assumption here is that the viewer has access more details os the shared scene with higher level of fidelity (e.g. 360 or 3D image) than a lower level of fidelity (2D image). 
