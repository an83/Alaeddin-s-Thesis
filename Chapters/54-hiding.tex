...
The user study could have used for a the viewer a VR headset such as HTC Vive. However an AR device was used instead for 2 reasons: 1) to allow the Viewer to navigate their way inside the shared remote environment while still being aware of their physical environment. This will open questions for future research directions addressing issues of non-similar environments (between Sharer and Viewer) – which is closer to real scenarios for social sharing purposes. 
2) the focus of this study is to look at hiding mechanisms. In VR setup, the physical world is not visible, therefore when an item is hidden, the Viewer will see a blank (in the Remove case). However in AR setup, the Viewer will see the physical world instead.
