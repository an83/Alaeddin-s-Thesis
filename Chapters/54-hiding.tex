...
This study focused on symmetric setup (AR headset for both users) between the Sharer and the Viewer. The reasons for enabling the viewer to see their surrounding environment (rather than use a VR headset) are: 1) to allow the Viewer to navigate their way inside the shared remote environment while still being aware of their physical environment. This will open questions for future research directions addressing issues of non-similar environments (between Sharer and Viewer) – which is closer to real scenarios for social sharing purposes. 2) the focus of this study is to look at hiding mechanisms. In VR setup, the physical world is not visible, therefore when an item is hidden, the Viewer will see a blank (in the Remove case). However, in an AR setup, the Viewer will see the physical world instead.
